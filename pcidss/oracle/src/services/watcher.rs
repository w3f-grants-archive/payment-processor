		...
		if event.pallet_name().contains("ISO8583") {
			...
			match event_name {
				x if x.contains("InitiateTransfer") => {
					...
					let from_bank_account =
						from_bank_account?.ok_or("From bank account not found")?;
				...
				x if x.contains("InitiateRevert") => {
					...
					let (who_hex, hash_hex): (String, String) = (
						who.0.iter().map(|b| format!("{:02x}", b)).collect(),
					...
					let (from_bank_account, maybe_transaction) = futures::join!(
						self.processor
							.bank_account_controller
							.find_by_account_id(&who_hex.as_str()),
					...
					self.submit_finality(
						who,
						AccountId32(
							PALLET_ACCOUNT.as_bytes().try_into().expect("valid account;qed"),
						),
						0,
						iso_msg,
						&event_id,
